...
static void printUsage11(char** argv)
{
    cout <<
...
//string warp_type = "plane";//"plane";
string warp_type = "plane";
// plane | cylindrical | spherical  | paniniA2B1)\n"
int expos_comp_type = ExposureCompensator::CHANNELS_BLOCKS;
...
static int parseCmdArgs11(int argc, char** argv)
{
    if (argc == 1)
    {
        printUsage11(argv);
...
void find_jpg_files11(const std::filesystem::path& dir, std::vector<std::string>& files) {
    for (const auto& entry : std::filesystem::directory_iterator(dir)) {
        if (entry.is_regular_file() && (entry.path().extension() == ".JPG" || entry.path().extension() == ".jpg")) {
            files.push_back(entry.path().string());
        }
        else if (entry.is_directory()) {
            find_jpg_files11(entry.path(), files); // µÝ¹éµ÷ÓÃ
        }
    }
}
...
#endif
    vector<string> img_names;
    find_jpg_files11("E://PROJECT//stitcher//demo3x3//001_0003", img_names);
